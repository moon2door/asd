...
namespace Device
{
	namespace Panda40p
	{
		...
		void Convert2PointCloud(pcl::PointCloud<pcl::PointXYZ> &dst, Device::Panda40p::StLidarData* src)
		{
			pcl::PointCloud<pcl::PointXYZ> tmp;
			tmp.reserve(40 * 10);
			for (int idxBlock = 0; idxBlock < 10; idxBlock++)
			{
				double  stdAngle = src->dataBlock[idxBlock].azimuth * 0.01; // -- 기준 각도(deg)

				for (int idxLayer = 0; idxLayer < 40; idxLayer++)
				{
		...
		void Convert2PointCloud(pcl::PointCloud<pcl::PointXYZI> &dst, Device::Panda40p::StLidarData* src)
		{
